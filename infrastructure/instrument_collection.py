...
class InstrumentCollection:
    ...
    def LoadInstrumentsDB(self):
        self.instruments_dict = {}
        data = DataDB().query_single(DataDB.INSTRUMENTS_COLLECTION)
        for k,v in data.items():
                self.instruments_dict[k] = Instrument.FromApiObject(v)
    ...
    def CreateDB(self,data):
        ...
        database = DataDB()
        database.delete_many(DataDB.INSTRUMENTS_COLLECTION)
        database.add_one(DataDB.INSTRUMENTS_COLLECTION, instruments_dict)
